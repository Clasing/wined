...
import { jwtAuth } from './middleware/clerk.js';
...
// Public auth endpoints (register/login/refresh/logout) — no JWT required.
...
// /api/v1/auth/me is protected. authRoute already serves /me, but it lives in
// the public chain; mount a jwt-protected variant explicitly so the middleware
// runs before the handler.
const protectedMe = new Hono();
protectedMe.use('*', jwtAuth());
protectedMe.get('/', (c) => {
  const auth = getAuth(c);
  return c.json({
    user: { id: auth.userId, email: auth.email, role: auth.role },
    org: { id: auth.orgId, product: auth.product },
  });
});
app.route('/api/v1/auth/me', protectedMe);
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  if (process.env['WINED_START_WORKERS'] === '1') {
    ...
    const { startEmbeddingWorker } = await import('./workers/embedding-worker.js');
    const { startCuratorWorker } = await import('./workers/curator-worker.js');
    const { startDistributorCatalogWorker } = await import(
      './workers/distributor-catalog-worker.js'
    );
    const { startGdprExportWorker } = await import('./workers/gdpr-export-worker.js');
    const { startCitationValidatorWorker } = await import('./workers/citation-validator-worker.js');
    const { startStockPriceImportWorker } = await import('./workers/stock-price-import-worker.js');
  ...
  serve({ fetch: app.fetch, port }, (info) => {
    // eslint-disable-next-line no-console
    console.log(`[wined-api] listening on http://localhost:${info.port} (env=${env.NODE_ENV})`);
  });
}
